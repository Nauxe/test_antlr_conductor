...
export class Heap {
  ...
  // are used on the heap and the first child will point to the 
  ...
  set_child(addr: number, value: number) {
    this.data[addr + 5] = value;
  }

  // addr is the address of the start of a word
  get_child(addr: number): number {
    return this.data[addr + 5]
  }
}
